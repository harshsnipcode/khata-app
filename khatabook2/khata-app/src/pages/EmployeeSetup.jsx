...
function EmployeeSetup() {
  ...
  const tempData = JSON.parse(sessionStorage.getItem("temp_employee") || "null");
  if (!tempData) {
  ...
  const handleSave = async () => {
    ...
    navigate("/admin/home", { state: { activeTab: "employees" }, replace: true });
  };

  const createdDate = tempData.created_at
    ? new Date(tempData.created_at).toLocaleDateString("en-IN", {
        day: "2-digit",
        month: "short",
        year: "2-digit",
      })
      ...
      <div className="max-w-2xl mx-auto p-6 space-y-6 animate-fade-in">
        ...
        <div className="card rounded-3xl p-6 shadow-md flex items-center gap-4">
          <div className="w-14 h-14 rounded-full bg-[var(--primary-light)] border border-[var(--primary)]/20 flex items-center justify-center text-[var(--primary)] font-bold text-xl shrink-0">
            {tempData.username[0]?.toUpperCase() || "?"}
          </div>
          <div>
            <p className="text-[var(--text-primary)] font-bold text-lg">{tempData.username}</p>
            <p className="text-[var(--text-muted)] text-xs font-medium">Created {createdDate}</p>
          </div>
        </div>

        {/* Section title */}
        <p className="text-[var(--text-primary)] font-bold text-base -mb-3">Manage Salary & Permissions</p>

        {/* Toggle: Attendance & Salary */}
